...
调用者只需提供原图/清单，并说明要“从空白画布逐图元绘制”或“保真导入”，以及是否需要可编辑 DWG。不得要求调用者了解或在请求中写入 `1`、`CADLIVE`、`type_text`、Bundle 路径、LSP 加载或延时参数；这些全是本 Skill 的内部执行细节。当需要向调用者提供“调用指令”时，只保留业务目标、输入图纸和交付要求。
