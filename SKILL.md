...
这是自包含的 **Mac 原生适配**，无需安装其他 CAD 技能；不调用 `win32com`、`pywin32`、`taskkill`。豆包或其他代理执行本技能，须另有本地电脑操作能力且当前 Mac 的 AutoCAD 可交互；技能文字和离线文件本身不会自动获得桌面权限。目前脚本通过离线测试，AutoCAD 原生逐步绘制和 DWG 交付仍需实机验证。
