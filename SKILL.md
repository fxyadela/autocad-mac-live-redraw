...
description: 在 macOS 的本机 AutoCAD 中，把已校准的二维重绘清单按图元逐一画成可编辑 CAD 图形，默认在绘制完成后停留于未保存画布；或按用户明确要求将现有 DXF 另存为 DWG。适用于豆包工作等具备本地电脑操作能力的代理；不适用于 Windows COM 自动化。
---
...
调用者只需提供原图/清单，并说明要“从空白画布逐图元绘制”或“保真导入”。逐图元绘制默认在完成后停留于当前画布，**不保存、不另存、不关闭**；只有调用者明确要求 DWG 文件时才进入保存流程。不得要求调用者了解或在请求中写入 `1`、`CADLIVE`、`type_text`、Bundle 路径、LSP 加载、保存命令或延时参数；这些全是本 Skill 的内部执行细节。当需要向调用者提供“调用指令”时，只保留业务目标、输入图纸和交付要求。
...
7. **默认停止条件：**完成核对后停留在当前 AutoCAD 画布，不按 `Cmd+S`，不调用 `SAVE`/`QSAVE`/`SAVEAS`，不打开保存窗口，不关闭或重开图纸。向调用者报告“绘制完成，当前未保存”，然后停止操作。不得把“可能需要交付文件”自行解释为保存授权。
8. 只有调用者明确说“保存”“另存为 DWG”或提供保存路径时，才用 AutoCAD 的“另存为”选择 **DWG**，保存到新的绝对路径；如调用者同时要求验收，再关闭重开并实际选中/修改图元。保留原图、清单、LSP、DXF 和 DWG 的独立路径，不覆盖源文件。
9. 汇报分开列：清单校准与原图准确度、LSP/DXF 离线校验、AutoCAD 实际加载和逐步绘制、当前保存状态，以及用户明确要求后才执行的 DWG 保存/重开检验。每级缺证据就标“未验证”，不能靠日志或视频推断交付完成。
