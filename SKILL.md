...
2. 首次部署 bundle 后，在正式录制前正常重启 AutoCAD 一次；若出现第三方代码加载安全提示，由用户正常确认，不要关闭 `SECURELOAD` 或修改 `TRUSTEDPATHS`。后续更新当前 LSP 不需重复安装或重启，但必须在部署后新建一个空白图，使每文档加载拿到最新版。
3. 在空白图中只输入短命令 `CADLIVE`。不要输入任何以 `(` 开头的 AutoLISP 表达式，不要打开 `APPLOAD`，也不要在 `type_text`、`set_value`、逐键 `press_key` 之间反复重试；这些都是已确认会造成卡顿的同一路径。若短命令一次输入后仍未执行，停止并报告命令栏原文。
