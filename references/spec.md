...
`units`: `unitless`, `mm`, `cm`, `m`, `in`, `ft`；不能确定就写 `unitless`。点均是 `[x,y]` 或 `[x,y,z]`，绘制只支持模型空间平面（`z=0`）。角度是度。图元按 `entities` 顺序分批展示，在 JSON 中先排墙/结构，再门、窗、尺寸、标签。文本 `height` 为 CAD 单位，圆/弧半径必须大于零；`text` 单行，`mtext` 可多行且需要 `width`。
...
输出的 `.lsp` 需要在本机 AutoCAD 新建空白图经 APPLOAD 加载；`CADLIVE` 可见分批绘图，`CADFAST` 为无停顿核验。导出的 DWG 须另存、重开、实际选中图元和对照原图，离线编译成功不能替代这些检查。
