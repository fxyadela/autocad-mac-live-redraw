...
输出的 `.lsp` 需要先用 `scripts/deploy_autocad_bundle.py` 部署到本机 AutoCAD 用户级 bundle。`BUNDLE_UPDATED` 后只需新建空白图；`BUNDLE_INSTALLED`/`BUNDLE_UPGRADED` 且 AutoCAD 已打开时，用 `_APPAUTOLOADER` → `_Reload` 刷新一次插件，不要退出重启。命令栏回到“键入命令/Command:”后，让桌面代理只按单键 `K` 和 Return；不要用 `type_text`/`set_value` 输入完整 `CADLIVE`，否则文本截断为 `C` 时会误启 `CIRCLE`。不要输入长 Lisp 表达式，不要打开 `APPLOAD`、F1、帮助页或浏览器。命令不识别就立即停止报告。`K` 与人工键盘输入的 `CADLIVE` 都会预先定位视口并逐个显示原生图元，`CADFAST` 为无停顿核验。导出的 DWG 须另存、重开、实际选中图元和对照原图，离线编译成功不能替代这些检查。
