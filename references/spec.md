...
输出的 `.lsp` 需要先用 `scripts/deploy_autocad_bundle.py` 部署到本机 AutoCAD 用户级 bundle。`BUNDLE_UPDATED` 后只需新建空白图；`BUNDLE_INSTALLED`/`BUNDLE_UPGRADED` 且 AutoCAD 已打开时，由于 AutoCAD for Mac 没有 `_APPAUTOLOADER` 命令，须在正式绘图任务前正常关闭并启动一次；任务中不得将重启当成失败重试。单图用 `compile_mac_redraw.py`，两至九图用 `compile_mac_batch.py` 并按上传顺序重复 `--spec`；更多图片按顺序分组。命令栏空闲后，桌面代理只输入单键 `1` 并按 Return；当前图核对完成、仍有下一张时才用内部单键 `0` 调用 `QNEW`，确认空白后再输入 `1`。批量脚本用独立环境状态记录顺序，不需要为每张重新部署单图脚本。不要输入完整 `CADLIVE`、长 Lisp 表达式，也不要打开 `APPLOAD`、F1、帮助页或浏览器。数字命令或新建图纸失败就立即停止报告。逐图元演示默认保留每张未保存图纸，最后一张完成后才结束；只有用户明确要求 DWG 时才保存或重开，离线编译成功不能替代原生检查。
